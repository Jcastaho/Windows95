package com.straccion.windows95.extensions
...
private fun drawPatternBackground(scope: DrawScope) {
    val painterLightColor = Paint().apply {
        color = Color.White
    }
    scope.drawIntoCanvas { canvas ->
        ...
        for (y in 0 until scope.size.height.toInt() step size.toInt() * 2) {
            for (x in 0 until scope.size.width.toInt() step size.toInt() * 2) {
                canvas.drawRect(Rect(x.toFloat(), y.toFloat(), size + x, size + y), painterLightColor)
            }
        }
    }
}

fun Modifier.onrightClic(onClickPosition: (IntOffset) -> Unit): Modifier {
